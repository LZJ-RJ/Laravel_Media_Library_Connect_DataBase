...
//    use BootExtension;
    ...
    public function __construct($path = '/')
    {
        ...
        $this->fileTypes = config('media.file.types');
    ...
    {
        $disk = config('media.file.disk');

        $this->storage = Storage::disk($disk);

        if (!$this->storage->getDriver()->getAdapter() instanceof Local) {
            Handler::error('Error', '[module/media] only works for local storage.');
        }
    }
    ...
    public function urls()
    {
        return [
            ...
            'move'       => route('admin.media.move'),
            'delete'     => route('admin.media.delete'),
            'upload'     => route('admin.media.upload'),
            'new-folder' => route('admin.media.newFolder'),
        ];
    }
    ...
    public function formatFiles($files = [])
    {
        $files = array_map(function ($file) {
            return [
                'download'  => route('admin.media.download', compact('file')),
                'icon'      => '',
                'name'      => $file,
                'preview'   => $this->getFilePreview($file),
                'isDir'     => false,
                'size'      => $this->getFilesize($file),
                'link'      => route('admin.media.download', compact('file')),
    ...
    protected function previewLink($url)
    {
        return sprintf(
            '<a href="%s"><span class="file-icon text-aqua">' .
            '<i class="fa fa-folder"></i></span></a>',
            $url
        );
    }
    ...
    {
        $layout = request()->get('layout', 'index');
        ...
        $url = route('admin.media.index', $args);

        $preview = $this->previewLink($url);

        $dirs = array_map(function ($dir) use ($preview, $layout) {
            $link = route(
                'admin.media.index', [
                    'path' => '/' . trim($dir, '/'),
                    'view' => request('view'),
    ...
    public function navigation()
    {
        ...
        foreach ($folders as $folder) {
            ...
            $navigation[] = [
                'name'  => $folder,
                'url'   => route('admin.media.index', ['path' => $path]),
            ];
        }
