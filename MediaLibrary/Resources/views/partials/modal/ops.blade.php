<section>
        ...
        <div class="mm-animated fadeInDown __modal-content-wrapper">
            <transition :name="`mm-img-${imageSlideDirection}`"
                ...
                <div class="modal-content" :key="selectedFile.path">
                    {{-- card v --}}
                    @include('medialibrary::partials.card')
                </div>
            </transition>
        </div>
    ...
    <div v-if="isActiveModal('imageEditor_modal')"
        ...
        <div class="mm-animated fadeInDown __modal-content-wrapper">
            <image-editor route="{{ route('mediaLibrary.uploadCropped') }}"
                :no-scroll="noScroll"
                :file="selectedFile"
                :trans="trans">
            </image-editor>
        ...
        <div class="modal-card mm-animated fadeInDown">
            <header class="modal-card-head is-black">
                <p class="modal-card-title">
                    <span>{{ trans('medialibrary::messages.save.link') }}</span>
                </p>
                <button type="button" class="delete" @click.stop="toggleModal()"></button>
            </header>

            <form action="{{ route('mediaLibrary.uploadLink') }}" @submit.prevent="saveLinkForm($event)">
                <section class="modal-card-body">
                    <input class="input" type="text"
                        v-model="urlToUpload"
                        placeholder="{{ trans('medialibrary::messages.add.url') }}"
                        ref="save_link_modal_input">
                </section>
                <footer class="modal-card-foot">
                    <button type="reset" class="button" @click.stop="toggleModal()">
                        {{ trans('medialibrary::messages.cancel') }}
                    </button>
                    <button type="submit"
                        class="button is-link"
                        :disabled="isLoading"
                        :class="{'is-loading': isLoading}">
                        {{ trans('medialibrary::messages.upload.main') }}
                    </button>
                </footer>
            </form>
        </div>
    ...
    <div class="modal mm-animated fadeIn"
        ...
        <div class="modal-card mm-animated fadeInDown">
            <header class="modal-card-head is-link">
                <p class="modal-card-title">
                    <span>{{ trans('medialibrary::messages.add.new_folder') }}</span>
                </p>
                <button type="button" class="delete" @click.stop="toggleModal()"></button>
            </header>

            <form action="{{ route('mediaLibrary.new_folder') }}" @submit.prevent="NewFolderForm($event)">
                <section class="modal-card-body">
                    <input class="input" type="text"
                        v-model="newFolderName"
                        placeholder="{{ trans('medialibrary::messages.new.folder_name') }}"
                ...
                <footer class="modal-card-foot">
                    ...
                    <button type="submit"
                        ...
                        :disabled="isLoading"
                        :class="{'is-loading': isLoading}">
                        {{ trans('medialibrary::messages.new.create_folder') }}
                    </button>
                </footer>
            </form>
        </div>
    ...
    <div class="modal mm-animated fadeIn"
        ...
        <div class="modal-card mm-animated fadeInDown">
            <header class="modal-card-head is-warning">
                <p class="modal-card-title">
                    <span>{{ trans('medialibrary::messages.rename.file_folder') }}</span>
                </p>
                <button type="button" class="delete" @click.stop="toggleModal()"></button>
            </header>

            <form action="{{ route('mediaLibrary.rename_file') }}" @submit.prevent="RenameFileForm($event)">
                <section class="modal-card-body">
                    <h3 class="title">{{ trans('medialibrary::messages.new.file_folder') }}</h3>
                    <input class="input" type="text"
                        v-if="selectedFile"
                        v-model="newFilename"
                ...
                <footer class="modal-card-foot">
                    ...
                    <button type="submit"
                        ...
                        :disabled="isLoading"
                        :class="{'is-loading': isLoading}">
                        {{ trans('medialibrary::messages.rename.main') }}
                    </button>
                </footer>
            </form>
        </div>
    ...
    <div class="modal mm-animated fadeIn"
        ...
        <form class="modal-card mm-animated fadeInDown"
            action="{{ route('mediaLibrary.move_file') }}"
            ...
            <header class="modal-card-head is-warning">
                <p class="modal-card-title">
                    ...
                    <transition name="mm-list" mode="out-in">
                        <span key="1" v-if="copyFilesNotMove">{{ trans('medialibrary::messages.copy.file_folder') }}</span>
                        <span key="2" v-else>{{ trans('medialibrary::messages.move.file_folder') }}</span>
                    </transition>
                </p>
                <button type="button" class="delete" @click.stop="toggleModal()"></button>
            </header>
            ...
            <section class="modal-card-body">
                ...
                <h5 class="subtitle m-b-20">
                    {{ trans('medialibrary::messages.destination_folder') }} :
                    <span class="tag is-black is-medium">@{{ files.path || '/' }}</span>
                </h5>

                @include('medialibrary::partials.modal.mov-files-info')
            </section>
            ...
            <footer class="modal-card-foot">
                <div class="level is-mobile full-width">
                    ...
                    <div class="level-left">
                        ...
                        <div class="level-item">
                            <label class="label" for="use_copy">{{ trans('medialibrary::messages.copy.files') }}</label>
                        </div>
                    </div>
                    ...
                    <div class="level-right">
                        ...
                        <div class="level-item">
                            <button type="reset"
                                ...
                                {{ trans('medialibrary::messages.crop.reset') }}
                            </button>
                        </div>
                        <div class="level-item">
                            <button type="submit"
                                ...
                                :disabled="isLoading"
                                :class="{'is-loading': isLoading}">
                                <span>{{ trans('medialibrary::messages.paste') }}</span>
                            </button>
                        </div>
                    </div>
                </div>
    ...
    <div class="modal mm-animated fadeIn"
        ...
        <div class="modal-card mm-animated fadeInDown">
            <header class="modal-card-head is-danger">
                <p class="modal-card-title">
                    <span>{{ trans('medialibrary::messages.are_you_sure_delete') }}</span>
                </p>
                <button type="button" class="delete" @click.stop="toggleModal()"></button>
            </header>

            <form action="{{ route('mediaLibrary.delete_file') }}"
                @submit.prevent="DeleteFileForm($event)">
                <section class="modal-card-body">
                    @include('medialibrary::partials.modal.del-files-info')
                    ...
                    <h5 v-show="folderDeleteWarning" class="__modal-folder-warning">
                        ...
                        <span>{{ trans('medialibrary::messages.delete.folder') }}</span>
                    </h5>
                </section>
                ...
                <footer class="modal-card-foot">
                    ...
                    <button type="submit"
                        ...
                        :disabled="isLoading"
                        :class="{'is-loading': isLoading}">
                        {{ trans('medialibrary::messages.delete.confirm') }}
                    </button>
                </footer>
            </form>
        </div>
